...
def make_sentiment(value):
    ...
    sentiment = {'key': result, 'value': value}
    return sentiment
...
def find_centroid(polygon):
    ...
    while x < length:
        sums = latitude(polygon[x]) * longitude(polygon[x + 1]) - \
            latitude(polygon[x + 1]) * longitude(polygon[x])
        summation.append(sums)
        xcoor.append(
            (latitude(polygon[x]) + latitude(polygon[x + 1])) * sums)
        ycoor.append(
            (longitude(polygon[x]) + longitude(polygon[x + 1])) * sums)
        x += 1
    area = 0.5 * (sum(summation))
    if area == 0:
        return latitude(polygon[0]), longitude(polygon[0]), 0
    x_coordinate = sum(xcoor) / (6 * area)
    y_coordinate = sum(ycoor) / (6 * area)
...
def find_state_center(polygons):
    ...
    centroids = []
    for p in polygons:
        centroids.append(find_centroid(p))
    latitude = []
    longitude = []
    area = []
    for x, y, z, in centroids:
        latitude.append(x * z), longitude.append(y * z), area.append(z)
    xcoor = sum(latitude) / sum(area)
    ycoor = sum(longitude) / sum(area)
    return make_position(xcoor, ycoor)
    ...
    for tweet in tweets:
        ...
        for state in us_states:
            if geo_distance(position, find_state_center(us_states[state])) < min:
                min = geo_distance(
                    position, find_state_center(us_states[state]))
                result_state = state
...
def draw_centered_map(center_state='TX', n=10):
    ...
    def dist_from_center(name): return geo_distance(center, us_centers[name])
